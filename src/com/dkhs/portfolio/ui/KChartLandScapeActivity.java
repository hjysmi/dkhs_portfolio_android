...
public class KChartLandScapeActivity extends FragmentActivity implements OnClickListener, ITouchListener, Serializable {
	...
	private int								type;
	private String							checkValue					= PortfolioApplication.getInstance()
																				.getCheckValue();
	...
	ParseHttpListener		listener			= new ParseHttpListener<StockQuotesBean>() {
													...
													protected StockQuotesBean parseDateTask(String jsonData) {
														...
														try {
															...
															stockQuotesBean =
																	DataParse.parseObjectJson(
																			StockQuotesBean.class, jsonOb);
															if (null != stockQuotesBean
																	&& UIUtils.roundAble(stockQuotesBean)) {
																quoteHandler.removeCallbacks(runnable);
															}
															List<FiveRangeItem> buyList =
																	new ArrayList<FiveRangeItem>();
															List<FiveRangeItem> sellList =
																	new ArrayList<FiveRangeItem>();
															int i = 0;
															for (; i < 5; i++) {
																// String buyPrice :
																...
																if (i < stockQuotesBean.getBuyPrice().getBuyVol()
																		.size()) {
																	String buyPrice =
																			stockQuotesBean.getBuyPrice().getBuyPrice()
																					.get(i);
																	...
																	String volText =
																			stockQuotesBean.getBuyPrice().getBuyVol()
																					.get(i);
															...
															for (int j = 4; j >= 0; j--) {
																...
																if (j < stockQuotesBean.getSellPrice().getSellVol()
																		.size()) {
																	String sellPrice =
																			stockQuotesBean.getSellPrice()
																					.getSellPrice().get(j);
																	...
																	String sellVol =
																			stockQuotesBean.getSellPrice().getSellVol()
																					.get(j);
													...
													protected void afterParseData(StockQuotesBean object) {
														...
														if (null != object) {
															...
															mStockQuotesChartFragment
																	.setStockQuotesBean(mStockQuotesBean);
															landKlinTextTitle.setText(object.getName());
															landKlinTextPrice.setText(object.getCurrent() + "");
															landKlinTextPrice.setTextColor(ColorTemplate
																	.getUpOrDrownCSL(object.getPercentage()));
															landKlinTextValum.setText(UIUtils.getshou(object
																	.getVolume()));
															landKlinTextData.setText(TimeUtils.getTimeString(object
																	.getMoment()));
														}
													}
												};
	...
	private void initTabPage() {
		...
		mStockQuotesChartFragment =
				StockQuotesChartLandFragment.newInstance(StockQuotesChartLandFragment.TREND_TYPE_TODAY, mStockCode);
		mStockQuotesChartFragment.setITouchListener(this);
		fragmentList.add(mStockQuotesChartFragment);
		KChartsLandFragment fragment =
				KChartsLandFragment.getKChartFragment(KChartsLandFragment.TYPE_CHART_DAY, mStockCode, symbolType);
		fragment.setITouchListener(this);
		fragmentList.add(fragment);
		KChartsLandFragment fragment2 =
				KChartsLandFragment.getKChartFragment(KChartsLandFragment.TYPE_CHART_WEEK, mStockCode, symbolType);
		fragment2.setITouchListener(this);
		fragmentList.add(fragment2);
		KChartsLandFragment fragment3 =
				KChartsLandFragment.getKChartFragment(KChartsLandFragment.TYPE_CHART_MONTH, mStockCode, symbolType);
		...
		if (resultCode == RESULT_OK) {
			...
			switch (requestCode) {
				case REQUESTCODE_SELECT_STOCK:
					SelectStockBean selectBean =
							(SelectStockBean) data.getSerializableExtra(FragmentSelectStockFund.ARGUMENT);
					if (null != selectBean) {
