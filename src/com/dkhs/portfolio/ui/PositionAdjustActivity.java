 ...
 * @version 1.0
 */
public class PositionAdjustActivity extends ModelAcitivity implements
		IDutyNotify, OnClickListener {
	...
	private void handleExtras(Bundle extras) {
		...
		mPositionDetailBean = (PositionDetail) extras
				.getSerializable(EXTRA_POSITIONDETAIL);
		if (null != mPositionDetailBean) {
			...
			new MyCombinationEngineImpl().queryCombinationDetail(
					mCombinationId, listener);
		}
...
//		initStockPercentView();
		...
		if (null != mPositionDetailBean) {
			...
			positionTextValue
					.setText(StringFromatUtils.get4Point(mPositionDetailBean
							.getPortfolio().getNetvalue()));
			positionTextValue.setTextColor(ColorTemplate
					.getUpOrDrownCSL(mPositionDetailBean.getPortfolio()
							.getNetvalue() - 1));
			// String time =
			// mPositionDetailBean.getPortfolio().getCreateTime().replace("T",
			// "-");
			// time = time.substring(0, time.length() - 4);
			positionTextCreatedate.setText("成立时间:"
					+ TimeUtils.getSimpleFormatTime(mPositionDetailBean
							.getPortfolio().getCreateTime()));
		}
		isShowAverageButton();
	}
	...
	private void initConbinationInfoView() {
		...
		if (isAdjustCombination) {
			...
			// tvCreateTime = (TextView)
			// inflatedView.findViewById(R.id.tv_create_time);
			// tvTodayNetvalue = (TextView)
			// inflatedView.findViewById(R.id.tv_today_netvalue);
			...
			// TextView tvCombinationName = (TextView)
			// inflatedView.findViewById(R.id.tv_portfolio_name);
			// tvCombinationName.setText(mPositionDetailBean.getPortfolio().getName());
			// }
			etConbinationName.setText(mPositionDetailBean.getPortfolio()
					.getName());
			etConbinationDesc.setText(mPositionDetailBean.getPortfolio()
					.getDescription());
		} else {
			setTitle(R.string.create_combination);
			// ViewStub viewstub = (ViewStub)
			// findViewById(R.id.create_portfolio_info);
	...
	private void setStockList() {
		...
		// System.out.println("PositionDetailBean.getCurrentDate():" +
		// mPositionDetailBean.getCurrentDate());
		//
		// ConStockBean stock1 = new ConStockBean(1, 0.3f,
		// getResources().getColor(ColorTemplate.DEFAULTCOLORS[0]),
		// "沪深大盘", "600123");
		// ConStockBean stock2 = new ConStockBean(2, 0.4f,
		// getResources().getColor(ColorTemplate.DEFAULTCOLORS[1]),
		// "苏宁云商", "622123");
		// ConStockBean stock3 = new ConStockBean(3, 0.3f,
		// getResources().getColor(ColorTemplate.DEFAULTCOLORS[2]),
		// "阿里巴巴", "666666");
		// // ConStockBean stock4 = new SurpusStock(surValue);
	...
	private void initFooterView() {
		mFooterView = View
				.inflate(this, R.layout.layout_optional_percent, null);
		...
		TextView tvName = (TextView) mFooterView
				.findViewById(R.id.tv_stock_name);
		tvName.setText(R.string.surplus_capital_ratio);
		tvSurpusValue = (TextView) mFooterView
				.findViewById(R.id.tv_stock_percent);
		tvSurpusValue.setText(StringFromatUtils
				.getPercentValue((int) (surValue)) + "");
		ScaleDrawable sd = (ScaleDrawable) ((LayerDrawable) surSeekbar
				.getProgressDrawable())
				.findDrawableByLayerId(android.R.id.progress);

		GradientDrawable gd = (GradientDrawable) sd.getDrawable();
		gd.setColor(ColorTemplate.DEF_RED);
		mFooterView.findViewById(R.id.view_color).setBackgroundColor(
				ColorTemplate.DEF_RED);
		// return foot;
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_right: {
			...
			// Toast.makeText(PositionAdjustActivity.this, "确定添加",
	...
	private void adjustPositionDetailToServer() {
		...
		isModiyName = !nameText.equalsIgnoreCase(mPositionDetailBean
				.getPortfolio().getName())
				|| !descText.equalsIgnoreCase(mPositionDetailBean
						.getPortfolio().getDescription());
		...
		if (isModiyName)
			engine.updateCombination(mCombinationId, nameText, descText,
					adjustNameListener);
	...
	ParseHttpListener adjustNameListener = new ParseHttpListener() {
		...
		protected void afterParseData(Object object) {
			mPositionDetailBean.getPortfolio().setName(
					etConbinationName.getText().toString());
			mPositionDetailBean.getPortfolio().setDescription(
					etConbinationDesc.getText().toString());
			Toast.makeText(PositionAdjustActivity.this, "名称修改成功",
					Toast.LENGTH_SHORT).show();
		}
	...
	ParseHttpListener adjustPositionListener = new ParseHttpListener() {
		...
		@Override
		protected void afterParseData(Object object) {
			Toast.makeText(PositionAdjustActivity.this, "持仓调整成功",
					Toast.LENGTH_SHORT).show();
			finish();
		}
	...
	private List<SubmitSymbol> generateSymbols() {
		...
		if (null != mPositionDetailBean) {
			...
			List<ConStockBean> originalBeanList = mPositionDetailBean
					.getPositionList();
		...
		for (ConStockBean stock : tempList) {
			...
			// System.out.println("symbols stock id:" + symbol.getSymbol() +
			// " value:" + symbol.getPercent());
			symbols.add(symbol);
		}
		return symbols;
	}
	...
	private void createCombinationByServer() {
		...
		if (symbolsList.size() < 1) {
			...
			new MyCombinationEngineImpl().createCombination(combinationName,
					combinationDesc, symbolsList,
					new ParseHttpListener<CombinationBean>() {
						...
						protected void afterParseData(CombinationBean object) {
							if (null != object && !isAdjustCombination) {
								PositionAdjustActivity.this
										.startActivity(CombinationDetailActivity
												.newIntent(
														PositionAdjustActivity.this,
														object));
							}
						...
						protected CombinationBean parseDateTask(String jsonData) {
							return DataParse.parseObjectJson(
									CombinationBean.class, jsonData);
	...
	private void submitAdjustToServer() {
		// new MyCombinationEngineImpl().adjustCombination(mCombinationId,
		// symbols, new BasicHttpListener() {
		//
		// @Override
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			...
			case REQUESTCODE_SELECT_STOCK:
				...
				int createType = data.getIntExtra(
						BaseSelectActivity.ARGUMENT_CRATE_TYPE,
	...
	private void setAddStockBack(List<SelectStockBean> listStock) {
		...
		for (SelectStockBean selectBean : listStock) {
			...
			if (stockList.contains(csBean)) {
				...
				// System.out.println("stockList.get(index).getPercent():" +
				// stockList.get(index).getPercent());
				// System.out.println("stockList.get(index).getDutyValue():" +
				// stockList.get(index).getDutyValue());
				// if (i < stockList.size()) {
	...
	class QueryCombinationDetailListener extends
			ParseHttpListener<PositionDetail> {
