...
//		mTabTitle = null;
	...
	private void drawCandleDetails(Canvas canvas) {
		...
		if (showDetails && mStartX > 3) {
			...
			if(mOHLCData.size() < MIN_CANDLE_NUM){
				selectIndext = (int) ((width - 2.0f - mStartX - addNum * (mCandleWidth + CANDLE_PADDING) ) / (mCandleWidth + CANDLE_PADDING) + mDataStartIndext);
			}
			...
			if(mOHLCData.size() < MIN_CANDLE_NUM){
				startX = (float) (width - 3 - (mCandleWidth + CANDLE_PADDING) * (selectIndext - mDataStartIndext) - (mCandleWidth - 1) / 2 - addNum * (mCandleWidth + CANDLE_PADDING));
			}
			...
			try {
				...
				if(!isB){
					canvas.drawText(new DecimalFormat("0.00").format(open), left + 1
							+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight + textMargin * 2,
							textPaint);
				}else{
			...
			canvas.drawText(new DecimalFormat("0.00").format(high), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight * 2+ textMargin * 3,
					textPaint);
			}else{
			...
			canvas.drawText(new DecimalFormat("0.00").format(low), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight * 3+ textMargin * 4,
					textPaint);
			}else{
			...
			canvas.drawText(new DecimalFormat("0.00").format(close), left + 1
					+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight * 4+ textMargin * 5,
					textPaint);
			}else{
			...
			try {
				...
				canvas.drawText(new DecimalFormat("0.00").format(mOHLCData.get(selectIndext).getChange()), left + 1
						+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight * 5+ textMargin * 6,
						textPaint);
				}else{
			...
			try {
				canvas.drawText(new DecimalFormat("0.00").format(mOHLCData.get(selectIndext).getPercentage()) + "%", left + 1
						+ DEFAULT_AXIS_TITLE_SIZE * 2.5f, top + textTextHeight * 6+ textMargin * 7,
						textPaint);
			} catch (Exception e) {
				canvas.drawText("--", left + 1 + DEFAULT_AXIS_TITLE_SIZE * 3.5f, top
						+ DEFAULT_AXIS_TITLE_SIZE * 6.0f, textPaint);
	...
	private void drawUpperRegion(Canvas canvas) {
		...
		try {
			...
			mCandleWidth = (width - 4) / 10.0 * 10.0 / mShowDataNum - CANDLE_PADDING;
			double rate = (getUperChartHeight() - 2) / (mMaxPrice - mMinPrice);
			if(mOHLCData.size() >= MIN_CANDLE_NUM){
				for (int i = 0; i < mShowDataNum && mDataStartIndext + i < mOHLCData.size(); i++) {
					...
					float left = (float) (width - 2 - mCandleWidth * (i + 1) - i*CANDLE_PADDING + PADDING_LEFT);
					float right = (float) (width - 3 - mCandleWidth * i - i*CANDLE_PADDING + PADDING_LEFT);
					float startX = (float) (width - 3 - mCandleWidth * i - (mCandleWidth - 1) / 2 - i * CANDLE_PADDING + PADDING_LEFT);
					if (open < close) {
					...
					for (int i = 0; i < mShowDataNum
						...
						if (i != 0) {
							canvas.drawLine(
									...
									(float) (width - 2 - (CANDLE_PADDING + mCandleWidth) * i - mCandleWidth * 0.5f + PADDING_LEFT),
				...
				for (int j = 0; j < MALineData.size(); j++) {
					...
					paint.setAntiAlias(true);
					paint.setTextSize( getResources().getDimensionPixelOffset(R.dimen.title_text_font));
					int selectIndext = (int) ((width - 2.0f - mStartX - mCandleWidth * addNum - CANDLE_PADDING * addNum) / (mCandleWidth + CANDLE_PADDING) + mDataStartIndext);
					...
					for (int i = 0; i < mShowDataNum
						...
						if (i != 0) {
							canvas.drawLine(
									startX + PADDING_LEFT,
									startY + DEFAULT_AXIS_TITLE_SIZE + 4,
									(float) (width - 2 - (CANDLE_PADDING + mCandleWidth) * (i + addNum) - mCandleWidth * 0.5f + PADDING_LEFT),
									(float) ((mMaxPrice - lineEntity.getLineData()
											.get(mDataStartIndext + i)) * rate + DEFAULT_AXIS_TITLE_SIZE + 4),
									paint);
						}
