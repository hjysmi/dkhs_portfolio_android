 ...
 * @version 1.0
 */
public class FragmentSelectStockFund extends Fragment implements
		ISelectChangeListener, OnClickListener, OnLoadMoreListener {
	private static final String TAG = FragmentSelectStockFund.class
			.getSimpleName();
...
//	private View mFootView;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (isItemClickBack) {
			mAdapterConbinStock = new AddStockItemAdapter(getActivity(),
					mDataList);
		} else if (isFund) {
			mAdapterConbinStock = new SelectCompareFundAdatper(getActivity(),
					mDataList);
		} else if (mViewType == ViewType.STOCK_OPTIONAL_PRICE.typeId) {
			mAdapterConbinStock = new OptionalPriceAdapter(getActivity(),
					mDataList);
		} else if (mViewType == ViewType.STOC_INDEX_MARKET.typeId) {
			mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(),
					mDataList);
			...
			mAdapterConbinStock = new SelectStockAdatper(getActivity(),
					mDataList);
	...
	private void loadDataByFund() {
		...
		if (mViewType == ViewType.FUND_MAININDEX.typeId) {
			...
			mLoadDataEngine = new FundDataEngine(mSelectStockBackListener,
					FundDataEngine.TYPE_INDEX);
		} else if (mViewType == ViewType.FUND_STOCK.typeId) {
			mLoadDataEngine = new FundDataEngine(mSelectStockBackListener,
					FundDataEngine.TYPE_STOCK);
	...
	private void loadDataByStock() {
		...
		if (mViewType == ViewType.STOCK_OPTIONAL.typeId) {
			mLoadDataEngine = new OptionalStockEngineImpl(
					mSelectStockBackListener, false);
		} else if (mViewType == ViewType.STOCK_OPTIONAL_PRICE.typeId) {
			mLoadDataEngine = new OptionalStockEngineImpl(
					mSelectStockBackListener, true);
		} else if (mViewType == ViewType.STOCK_HANDOVER.typeId) {
			mLoadDataEngine = new QuetosStockEngineImple(
					mSelectStockBackListener,
					QuetosStockEngineImple.ORDER_TURNOVER);
		} else if (mViewType == ViewType.STOCK_DRAWDOWN.typeId) {
			mLoadDataEngine = new QuetosStockEngineImple(
					mSelectStockBackListener, QuetosStockEngineImple.ORDER_DOWN);

		} else if (mViewType == ViewType.STOC_INDEX_MARKET.typeId) {
			mLoadDataEngine = new MarketCenterStockEngineImple(
					mSelectStockBackListener, MarketCenterStockEngineImple.DESC);
		} else if (mViewType == ViewType.STOC_INDEX_MARKET_CURRENT.typeId) {
			mLoadDataEngine = new MarketCenterStockEngineImple(
					mSelectStockBackListener,
			...
			mLoadDataEngine = new MarketCenterStockEngineImple(
					mSelectStockBackListener, MarketCenterStockEngineImple.ACE);
		} else if (mViewType == ViewType.STOC_INDEX_POSITION.typeId) {
			mLoadDataEngine = new OpitionCenterStockEngineImple(
					mSelectStockBackListener, MarketCenterStockEngineImple.DESC);
		} else if (mViewType == ViewType.STOC_INDEX_POSITION_ACE.typeId) {
			mLoadDataEngine = new OpitionCenterStockEngineImple(
					mSelectStockBackListener, MarketCenterStockEngineImple.ACE);
	...
	public void refresh() {
		...
		if (mLoadDataEngine != null
				&& UIUtils.roundAble(mLoadDataEngine.getStatu())
				&& !isLoadingMore) {
			// mDataList.clear();
			isLoading = true;
			mLoadDataEngine.loadData();
		...
		if (mLoadDataEngine != null && !isLoadingMore) {
			...
			if (UIUtils.roundAble(mLoadDataEngine.getStatu())) {
				...
				if ((mViewType == ViewType.STOC_INDEX_MARKET_CURRENT.typeId
						...
						&& null != mDataList) {
					((MarketCenterStockEngineImple) mLoadDataEngine)
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// R.layout.fragment_selectstock, true);
		// initView(view);
...
//				R.layout.layout_loading_more_footer, null);
	...
	OnItemClickListener priceStockItemClick = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			getActivity().startActivity(
					StockQuotesActivity.newIntent(getActivity(), itemStock));
		}
	};
	...
	private final String mPageName = PortfolioApplication.getInstance()
			.getString(R.string.count_stock_select);
