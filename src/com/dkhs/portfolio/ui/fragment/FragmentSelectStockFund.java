...
public class FragmentSelectStockFund extends Fragment implements ISelectChangeListener, OnClickListener,
    ...
    public enum ViewType {
        ...
        MARKET_STOC_INDEX(9),
        // 行情中心 国内指数不排序
        MARKET_STOC_INDEX_CURRENT(13),
        // 行情中心 指数排行榜查询 递减
        MARKET_STOC_INDEX_ACE(10),
        // 行情中心 个股排行榜查询 递减
        MARKET_STOC_INDEX_POSITION(11),
        // 行情中心 个股排行榜查询 递减
        MARKET_STOC_INDEX_POSITION_ACE(12),
        // 基金，主要指数
        FUND_MAININDEX(5),
        // 基金，指数
        FUND_INDEX(6),
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        } else if (mViewType == ViewType.MARKET_STOC_INDEX.typeId) {
            mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(), mDataList);
        } else if (mViewType == ViewType.MARKET_STOC_INDEX_CURRENT.typeId) {
            mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(), mDataList);
        } else if (mViewType == ViewType.MARKET_STOC_INDEX_ACE.typeId) {
            mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(), mDataList);
        } else if (mViewType == ViewType.MARKET_STOC_INDEX_POSITION.typeId) {
            mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(), mDataList);
        } else if (mViewType == ViewType.MARKET_STOC_INDEX_POSITION_ACE.typeId) {
            mAdapterConbinStock = new MarketCenterItemAdapter(getActivity(), mDataList);
        } else {
    ...
    public void refreshForMarker() {
        ...
        if (mLoadDataEngine != null && !isLoadingMore) {
            ...
            if (UIUtils.roundAble(mLoadDataEngine.getStatu())) {
                timeMill = 0;
                isRefresh = true;
                if ((mViewType == ViewType.MARKET_STOC_INDEX_CURRENT.typeId
                        || mViewType == ViewType.MARKET_STOC_INDEX.typeId || mViewType == ViewType.MARKET_STOC_INDEX_ACE.typeId)
                        && null != mDataList) {
                    ((MarketCenterStockEngineImple) mLoadDataEngine).loadDataFromCurrent(mDataList.size());
                }
                if ((mViewType == ViewType.MARKET_STOC_INDEX_POSITION.typeId || mViewType == ViewType.MARKET_STOC_INDEX_POSITION_ACE.typeId)
                        && null != mDataList) {
