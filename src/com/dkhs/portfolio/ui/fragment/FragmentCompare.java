...
public class FragmentCompare extends BaseFragment implements OnClickListener,
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			...
			ArrayList list = savedInstanceState
					.getParcelableArrayList("selectStockList");
			...
			System.out.println("savedInstanceState selectStockList size:"
					+ selectStockList.size());
			System.out.println("savedInstanceState mCompareItemList size:"
					+ mCompareItemList.size());
	...
	private void handleExtras(Bundle extras) {
		...
		mCreateCalender = TimeUtils
				.toCalendar(mCombinationBean.getCreateTime());
	...
	private void showShare() {
		...
		String customText = "这是我的基金「" + mCombinationBean.getName() + "」从"
				+ btnStartTime.getText() + "至" + btnEndTime.getText()
				+ "与公募基金的业绩PK结果。你也来创建属于你的基金吧。https://dkhs.com/portfolio/wap/";
	...
	public void onClick(View v) {
		...
		case R.id.btn_compare_fund: {
			if (isBetween7day()) {
				Toast.makeText(getActivity(), "查询时间范围太小，请不要小于7天",
						Toast.LENGTH_SHORT).show();
			} else {
	...
	private boolean isBetween7day() {
		long between_days = (cEnd.getTimeInMillis() - cStart.getTimeInMillis())
				/ (1000 * 3600 * 24);
	...
	private void requestCompare() {
		...
		mCompareEngine.compare(compareListener, mCompareIds,
				TimeUtils.getTimeString(cStart), TimeUtils.getTimeString(cEnd));
	...
	ParseHttpListener historyNetValueListener = new ParseHttpListener<HistoryNetValue>() {
		...
		protected HistoryNetValue parseDateTask(String jsonData) {
			...
			HistoryNetValue histroyValue = DataParse.parseObjectJson(
					HistoryNetValue.class, jsonData);
			return histroyValue;
		}
		...
		protected void afterParseData(HistoryNetValue object) {
			...
			if (object != null && isAdded()) {
				...
				if (dayNetValueList != null && dayNetValueList.size() > 0) {
					...
					mCombinationLine
							.setLineColor(ColorTemplate.MY_COMBINATION_LINE);
				...
				float increaseValue = (object.getEnd() - object.getBegin())
						/ object.getBegin();
				tvIncreaseValue.setText(StringFromatUtils
						.get2PointPercent((increaseValue) * 100));
				if (increaseValue > 0) {
	...
	private void getMaxOffetValue(HistoryNetValue historyNetValue) {
		...
		for (int i = 0; i < dataLenght; i++) {
			...
			if (dashLineSize == 0
					&& TimeUtils.simpleDateToCalendar(todayBean.getDate()) != null) {
				if (TimeUtils.simpleDateToCalendar(todayBean.getDate()).after(
						mCreateCalender)) {
					dashLineSize = i;
				}
			}
	...
	ParseHttpListener compareListener = new ParseHttpListener<List<LineEntity>>() {
		...
		protected List<LineEntity> parseDateTask(String jsonData) {
			...
			try {
				List<CompareFundsBean> beanList = DataParse.parseArrayJson(
						CompareFundsBean.class, new JSONArray(jsonData));
				...
				for (CompareFundsBean bean : beanList) {
					...
					float value = (bean.getEnd() - bean.getBegin())
							/ bean.getBegin();

					mCompareItemList.get(i).value = StringFromatUtils
							.get2PointPercent(value);
					i++;
				}
				maxNum = maxNum - baseNum;
				minNum = baseNum - minNum;
	...
	private void showPickerDate() {
		...
		if (isPickStartDate) {
			dpg = new DatePickerDialog(new ContextThemeWrapper(getActivity(),
					android.R.style.Theme_Holo_Light_Dialog_NoActionBar), null,
							...
							if (cStart.after(Calendar.getInstance())) {
								...
								if (isBeforeMonthCreateDate(cStart)) {
									...
									if (cStart.after(cEnd)) {
										PromptManager
												.showToast("查询开始时间不能大于查询结束时间");
										cStart = lastCalendar;
									} else {
										Calendar sevenAfter = (Calendar) cStart
												.clone();
										sevenAfter
												.add(Calendar.DAY_OF_MONTH, 7);
										if (cEnd.before(sevenAfter)) {
											PromptManager
													.showToast("查询间隔不能少于7天");
											cStart = lastCalendar;
										} else {
											needUpdate = true;
										}
		...
		dpg.setButton(DialogInterface.BUTTON_NEGATIVE, "取消",
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
					}
	...
	private void showBeforeCreateDayDialog() {
		...
		builder.setTitle(R.string.dialog_before_createday_title)
				.setItems(R.array.query_compare_type,
						new DialogInterface.OnClickListener() {
	...
	private boolean isBeforeMonthCreateDate(Calendar cStart) {
		Calendar beforeMonthCaleder = TimeUtils.toCalendar(mCombinationBean
				.getCreateTime());
		...
		if (resultCode == Activity.RESULT_OK) {
			...
			case REQUESTCODE_SELECT_FUND:
				...
				if (null != listStock) {
					...
					System.out.println("selectStockList size:"
							+ selectStockList.size());
