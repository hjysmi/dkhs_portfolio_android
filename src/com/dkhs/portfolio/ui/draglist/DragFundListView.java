 ...
 */
@SuppressLint("NewApi")
public class DragFundListView extends ListView {
    ...
    public DragFundListView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void review(int position) {
        ...
        if (null != adapter && adapter.getList().size() > 0) {
            List<CombinationBean> list = adapter.getList();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN && !isLock && !isMoving && !isDragItemMoving) {
            try {
                ...
                if (draggerIcon != null && x > draggerIcon.getLeft() && x < draggerIcon.getRight()
                ...
                View tv = dragger.findViewById(R.id.drag_text_delet_pad);
                if (!isMoving && (delete.getVisibility() == View.GONE)
                        && x < (this.getWidth() - btn.getWidth() - draggerIcon.getWidth()) && x > delete.getWidth()) {
    ...
    public List<CombinationBean> getList() {
        return ((DragFundListAdapter) getAdapter()).getList();
    }
    ...
    private void hideDropItem() {
        final DragFundListAdapter adapter = (DragFundListAdapter) this.getAdapter();
    ...
    private void testAnimation(int y) {
        final DragFundListAdapter adapter = (DragFundListAdapter) getAdapter();
        int tempPosition = pointToPosition(0, y);
