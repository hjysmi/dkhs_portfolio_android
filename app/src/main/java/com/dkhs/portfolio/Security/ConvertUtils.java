...
public abstract class ConvertUtils {
    ...
    public static java.util.Date getDate(int y, int m, int d, boolean inclusive) {
        ...
        Calendar c = Calendar.getInstance();
        c.clear();
        if (c.getActualMinimum(Calendar.YEAR) <= y && y <= c.getActualMaximum(Calendar.YEAR)) {
            c.set(Calendar.YEAR, y);
            if (c.getActualMinimum(Calendar.MONTH) <= m && m <= c.getActualMaximum(Calendar.MONTH)) {
                c.set(Calendar.MONTH, m);
                if (c.getActualMinimum(Calendar.DATE) <= d && d <= c.getActualMaximum(Calendar.DATE))
