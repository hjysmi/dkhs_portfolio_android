...
public class CompareIndexView   {
    ...
    public CompareIndexView(Context ctx,FundManagerInfoBean.AchivementsEntity mCombinationBean) {
        this.mCombinationBean = mCombinationBean;
        this.ctx = ctx;


        if(null != mCombinationBean.getEnd_date()){
            mCreateCalender = TimeUtils.simpleDateToCalendar(mCombinationBean.getEnd_date());
            cEnd = TimeUtils.simpleDateToCalendar(mCombinationBean.getEnd_date());
        }else{
            mCreateCalender=Calendar.getInstance();
        ...
        netValueEngine = new CompareEngine();
    ...
    public View initView() {
         ...
         View view=  LayoutInflater.from(ctx).inflate(setContentLayoutId(),null);
    ...
    ParseHttpListener historyNetValueListener = new ParseHttpListener<List<LineEntity>>() {
...
//                setXTitle(dayNetValueList);
    ...
    private void requestCompare() {
        ...
        maxOffsetValue = 0f;
        netValueEngine.compare(historyNetValueListener,mCombinationBean.getFund().getId()+"",TimeUtils.getTimeString(cStart), TimeUtils.getTimeString(cEnd)
                );
        mCompareEngine.compare(compareListener, mCompareIds, TimeUtils.getTimeString(cStart),
                TimeUtils.getTimeString(cEnd));
        compareListener.setLoadingDialog(ctx);
    }
